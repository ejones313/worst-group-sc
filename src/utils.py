...
class CSVAllOutputsLogger:
    ...
    def log(self, df):
        if self.first_time:
            df.to_csv(self.csv_path, header=True, mode='w')
            self.first_time = False
        else:
            df.to_csv(self.csv_path, header=False, mode='a')
...
def add_dropout(model, name = 'fc', dropout_prob = 0.1):
    last_layer = getattr(model, name)
    weights = last_layer.weight
    new_fc = nn.Sequential(nn.Dropout(dropout_prob), nn.Linear(last_layer.in_features, last_layer.out_features))
    new_fc[1].weight = weights
